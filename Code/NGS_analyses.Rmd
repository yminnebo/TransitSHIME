---
title: "16S rRNA gene amplicon analysis"
...
library(remotes)       # Package installation and dependency management
library(readxl)        # Read Excel files
library(ggplot2)       # Data visualization
library(ggfortify)     # Ordination plots
library(ggrepel)       # Repel overlapping text labels in plots
library(cowplot)       # Combine multiple plots into one
library(gganimate)     # Create animated plots
library(grid)          # Grid graphics
library(gridExtra)     # Arrange multiple grid-based plots
library(circlize)      # Circular visualization of data
library(pheatmap)      # Create pretty heatmaps
library(ComplexHeatmap)# Create complex heatmaps
library(pairwiseAdonis)# Post-hoc analysis of PerMANOVA
library(ggpubr)        # Add statistical information to ggplots
library(rstatix)       # Statistical analysis and visualization
library(reshape2)      # Reshape and aggregate data
library(plyr)          # Tools for data manipulation
library(gtable)        # Manipulate grid graphics
library(dplyr)         # Data manipulation and transformation
library(tidyr)         # Tidy data operations
library(tidyverse)     # Collection of packages for data manipulation and visualization
library(data.table)    # Data manipulation with data tables
library(cluster)       # Cluster analysis
library(gtools)        # Miscellaneous R programming tools
library(gdata)         # Various R programming tools for data manipulation
library(multcompView)  # Visualize multiple comparisons
library(lemon)         # Enhance ggplot2 visualizations
library(vegan)         # Community ecology package
library(phyloseq)      # Manipulate and analyze microbiome data
library(microbiome)    # Further manipulation of phyloseq data
library(ape)           # Analyses of phylogenetics and evolution
library(rBLAST)        # Interface for BLAST search
library(Biostrings)    # Manipulate biological strings
library(scales)        # Graphical scales for plots
library(stringr)       # String manipulation
library(RColorBrewer)  # Color palettes
library(openxlsx)      # Read and write Excel files
library(officer)       # Create PowerPoint presentations
library(foreach)       # Looping constructs for parallel computing
library(doParallel)    # Parallel computing in R

# Set the number of cores for parallel processing
corenumber = 10

# Load custom scripts
source('YM_KDP_papertheme.R')           # Custom theme for ggplot2
source('YM_changetaxa.R')               # Change taxa that don't correspond with RDP database
source("KDP_tax_summarize.R")           # Summarize taxonomic data
source("KDP_volcano.R")                 # Create volcano plots
source("KDP_splittaxfunction.R")        # Split taxonomic functions
source("YM_aggregate_top_OTU.R")        # Aggregate top OTUs
source("YM_fusetoplayer.R")             # Fuse plots into an animated player
source("YM_extrafunctions.R")           # Additional helper functions
...
# Universal colours for the project, selected by the author himself
source("YM_colours.R")

# Define labels for various variables
...
# Function to set the working directory for saving plots
saveplots <- function() setwd(INSERT DIRECTORY WHERE YOU WANT TO SAVE YOUR PLOTS HERE)

# Set variables RDP and otu
RDP <- TRUE
otu <- FALSE
```
...
if (RDP == TRUE) {
  ...
  taxtable <- x
  taxtable$ID2 <- taxtable$id
  taxtable$ID2 <- gsub("OTU", "", taxtable$ID2) 
  taxtable2 <- subset(taxtable, ID2 == "")
  taxtable2$ID2 <- c(32, 33, 34, 35, 36, 37, 38)
  taxtable <- subset(taxtable, ID2 != "")
  taxtable <- rbind(taxtable2, taxtable)
  taxtable$ID2 <- as.numeric(taxtable$ID2)
  test <- taxtable[duplicated(taxtable$ID2), ]
  taxtable <- arrange(taxtable, ID2)
  taxtable$ID2 <- str_pad(taxtable$ID2, 5, pad = "0")
  taxtable$ID2 <- paste0("Otu", taxtable$ID2, sep = "")
  row.names(taxtable) <- taxtable$ID2
  taxtable <- subset(taxtable, select = c(Kingdom, Phylum, Class, Order, Family, Genus))
  rm(x, y)
} else {
  taxfile <- "taxtable.taxonomy"
  test <- as.data.frame(tax_table(import_mothur(mothur_constaxonomy_file = taxfile)))
}

sharedfile = "counttable.shared"

# Load sampledata and alter it
sampledata <- read.csv2("Sampledata_Illu.csv", header = TRUE, sep = ",")
sampledata <- na.omit(sampledata)

## IMPORTANT, originally, donors 4 and 6 were the other way around. We decided to swap them around so that the graphs would be visually more pleasant: The original revalue donor 4 to 6 and 6 to 4
...
sampledata$Day <- as.numeric(sampledata$Day)
 ...
 ## Merge flow cytometry counts with sampledata
...
x <- as.data.frame(sample_names(mothur_data))

# Rename sample names of long transit in donors 3 and 6: 279-288 -> 281-290 and 379-388 -> 381-390
txx <- as.data.frame(phyloseq::otu_table(mothur_data), stringsAsFactors = FALSE)
colnames(txx)[colnames(txx) %in% c("279", "280", "281", "282", "283", "284", "285", "286", "287", "288",
                                    "379", "380", "381", "382", "383", "384", "385", "386", "387", "388")]  <- c("281", 
"282", "283", "284", "285", "286", "287", "288", "289", "290", "381", "382", "383", "384", "385", "386", "387", "388", "389", "390")
...
## Rename and remove "unclassified" from genus level in taxonomy of phyloseq 
...
## Calculate the percentage of OTUs present per genus
...
# Order 'highlist' by Abundance and extract 'Genus' column
sequence <- highlist[order(-highlist$Abundance), ]$Genus

# Calculate proportional counts in 'phylobj.QMP' and filter stable samples
...
# Create 'Top15phylobj.prop' and convert it to molten data frame
Top15.prop <- psmelt(Top15phylobj.prop <- phylobj.prop_stable)

# Convert 'Transit', 'PD_Colon', and 'Donor' factors to ordered levels
Top.prop <- Top15.prop
...
# Check if unique 'Genus' values in 'Top' are present in 'Top.prop'
...
gglist <- list()
gglist2 <- list()
df.grpPMP <- data.frame()
df.grpQMP <- data.frame()
statlist <- list()

# Display message about Kruskal-Wallis tests
message("Performing Kruskal-Wallis tests between transit times")

# Initialize data frames for pairwise comparisons and significant results
df.pwc <- data.frame()
df.pwc2 <- data.frame()
df.sig <- data.frame()

# Initialize lists for bar plot comparisons
comparisonlistbar <- list()
x <- 1
y <- 1

for (i in sequence) {
# Subset data for the specific genus in Top and Top.prop
Top1 <- subset(Top, Genus == i)
Top2 <- subset(Top.prop, Genus == i)

# Calculate statistics for Top1
T1 <- Top1 %>% group_by(Transit, PD_Colon) %>%
  summarise(SD = sd(Abundance), Abundance = mean(Abundance),
            Transit = Transit, PD_Colon = PD_Colon) 
T1 <- T1[!duplicated(T1), ]
T1$min <- T1$Abundance - T1$SD
T1$max <- T1$Abundance + T1$SD
max <- max(T1$max)
min <- min(T1$min)

# Create data frame with means and SD for Top1
T1 <- subset(Top1, Abundance > 0)
T1$Abundance <- as.numeric(T1$Abundance)
T1 <- T1 %>% group_by(Transit, PD_Colon) %>%
  summarise(SD = sd(Abundance), Abundance = mean(Abundance),
            Transit = Transit, PD_Colon = PD_Colon)
T1 <- T1[!duplicated(T1), ]
T1$Genus <- i
T1 <- subset(T1, select = c(Genus, Transit, PD_Colon, Abundance, SD))
df.grpQMP <- rbind(df.grpQMP, T1)

# Calculate statistics for Top2
T1 <- subset(Top2, Abundance > 0)
T1$Abundance <- as.numeric(T1$Abundance)
T1 <- T1 %>% group_by(Transit, PD_Colon) %>%
  summarise(SD = sd(Abundance), Abundance = mean(Abundance),
            Transit = Transit, PD_Colon = PD_Colon)
...
T1$Genus <- i
T1 <- subset(T1, select = c(Genus, Transit, PD_Colon, Abundance, SD))
df.grpPMP <- rbind(df.grpPMP, T1)

    
# Create a bar plot with absolute abundance
...
# Create another bar plot or box plot with absolute abundance
...
# Create a statistical plot for the data
plotstat <- statplot(dataframe = Top1, AboveZero = T, ymax = ymax) + theme(legend.position = "none")
ymax = max(ggplot_build(plotstat)$layout$panel_scales_y[[1]]$range$range)
ymin = min(ggplot_build(p)$layout$panel_scales_y[[1]]$range$range)
...
plot1 <- ggarrange(p + theme(legend.position = "none") + scale_y_continuous(limits = c(ymin, ymax)), 
                   plotstat, widths = c(1, 0.3))

# Calculate statistics and put them in a data frame
stats <- statplot(dataframe = Top1, AboveZero = T, plot = "ymax", showstat = F, yaxis = T)
Top5 <- subset(Top1, Abundance > 0)
Top5$Abundance <- as.numeric(Top5$Abundance)

  
 #### Proximal colon
 # For Proximal Colon
my_data5 <- subset(Top5, PD_Colon == "P")
unique_transit <- unique(my_data5$Transit)
my_data5$Transit <- droplevels(my_data5$Transit)
...
tryCatch(
  ...
  error = function(e) {
    skip_to_next <<- TRUE
    message(crayon::red("Only 1 transit time has", gen, "in the proximal colon! Skipped to next"))
  }
)
...
if (skip_to_next == FALSE) {
  ...
  message(pvaluep)
  effectsize2 <- Effectsize(my_data5)
} else {
  pvaluep <- 1
...
# For Distal Colon
my_data5 <- subset(Top5, PD_Colon == "D")
unique_transit <- unique(my_data5$Transit)
my_data5$Transit <- droplevels(my_data5$Transit)
...
tryCatch(
  ...
  error = function(e) {
    skip_to_next <<- TRUE
    message(crayon::red("Only 1 transit time has", gen, "in the distal colon! Skipped to next"))
  }
)
...
p2 <- p2 + facet_grid(PD_Colon ~ ., labeller = labeller(PD_Colon = PD_Colon.labs)) +
  ...
  theme(strip.background = element_rect(fill = "#D6E0F4"), strip.text.x = element_text(size = 20),
        strip.text.y = element_text(size = 20, angle = 270), legend.text = element_text(size = 18),
        legend.title = element_text(size = 18, face = "bold"), legend.title.align = 0.5) +
  xlab("Transit time") + ylab("Proportional abundance (%)") +
  guides(fill = guide_legend("Donor", ncol = 6)) + theme(strip.text.y = element_blank())
...
# Proportional Plots for Transit
...
plotstat <- statplot(dataframe = Top2, AboveZero = T, ymax = ymax)
boxlegend <- cowplot::get_legend(plotstat)
...
# Figure S12 - Mixomics - NGS and SCFA -sPLS
## To begin
```{r Starting with Mixomics, fig.height=10, fig.width=10}
subsetcolon = T; subsetvessel = "P"
otus <- F
...
## Figure S1C - Faecal samples
```{r FS, fig.width=20, fig.height=10, include = FALSE}
Top15 <- aggregate_top_taxa(phylobj.QMPFS, top= 15, "Genus")
Top15 <- subset_samples(Top15, PD_Colon == "P")


plot <- plot_bar(Top15, fill="Genus", x= "Ninetransit" 
...
Top25 <- subset_samples(Top25, PD_Colon == "P")
...
plot <- ggarrange(plotA, plot1, ncol = 2, common.legend = T, legend = "bottom")

ggsave(plot = plot , width = 20, height = 10, dpi = 300, filename = "Abundance_Faecal_samples.tiff")
```
...
# Figure 4 - Paperplot mean per day
...
unique_otus <- unique(Top15$OTU)
unique_otus_in_levels <- unique_otus %in% Top15levels

# Create a data frame to check if OTUs are labeled
x <- data.frame(x = unique_otus_in_levels, genus = unique_otus)
x$genus <- as.ordered(factor(x$genus, levels = Top15levels))
Top15$OTU <- as.ordered(factor(Top15$OTU, levels = Top15levels))

# Sort and define color scheme for visualization
desired_colors <- c("Acidaminococcus"="#DA5724", "Akkermansia"="#94cfd5", 
...
# Rename dates and factorize columns
...
plot <- ggplot(Top, aes(x = Day, y = Abundance, fill = Genus)) +
  geom_bar(position = "stack", stat = "identity") +
  facet_grid(PD_Colon ~ Donor + Transit, labeller = labeller(PD_Colon = PD_Colon.labsPC, Donor = donor.labs), scale = "free_x")
...
# Convert to DNA-style plot and return
plot <- Fusetoplayer(plot, Type = "DNA")
plot
...
plotFigure3 <- pgNew
saveplots()
ggsave(plot = plotFigure3 , width = 17, height = 11, dpi = 200, filename = "Figure4_lo_res.png")
ggsave(plot = plotFigure3 , width = 17, height = 11, dpi = 400, filename = "Figure4_hi_res.tiff")
```
...
if(BW == TRUE){plot <- plot + scale_fill_grey(name = "SHIME transit", start = 0.7, end = 1, labels= c("Short", "Medium", "Long")) + scale_color_grey(start = 0, end = 0.3) +scale_shape_manual(name = "Donor", values=c("1" = 15, "2" = 16, "3" = 17 , "4" = 18, "5" = 25, "6" = 9)) + guides(color = "none") + xlab("Transit")
 ...
 ### Statistics
    ...
    # pwc <- subset(pwc, p.adj.signif != "ns")
    if(nrow(subset(pwc, p.adj.signif != "ns"))>0){
...
## BH corrected
```{r}
df.res.kruskal.adj <- data.frame()

# kruskal tests
for(i in c("P", "D")){
  for(x in c("ABS", "PROP")){
  test <- subset(df.res.kruskal.all, vessel ==i); test <- subset(test, Abundance ==x)
  test <- test[grep("Otu0", test$Genus), ]
  test$p.adj <- p.adjust(as.numeric(test$p), method = "BH")
  
  df.res.kruskal.adj <- rbind(df.res.kruskal.adj, test)
  
  test <- subset(df.res.kruskal.all, vessel ==i); test <- subset(test, Abundance ==x)
  test <- test[!grepl("Otu", test$Genus),]
  test$p.adj <- p.adjust(as.numeric(test$p), method = "BH")
  df.res.kruskal.adj <- rbind(df.res.kruskal.adj, test)
  }
}

df.res.kruskal.adj$difference.p <- abs(df.res.kruskal.adj$p - df.res.kruskal.adj$p.adj)

test <- subset(df.res.kruskal.adj, p.adj < 0.05 & p < 0.05);test$different_result <- "no_different_outcome"
test2 <- subset(df.res.kruskal.adj, p.adj > 0.05 & p > 0.05); test2$different_result <- "no_different_outcome"
test3 <- subset(df.res.kruskal.adj, p.adj < 0.05 & p > 0.05); test3$different_result <- "different_outcome"
test4 <- subset(df.res.kruskal.adj, p.adj > 0.05 & p < 0.05); test4$different_result <- "different_outcome"
nrow(df.res.kruskal.adj)
nrow(test) + nrow(test2) +nrow(test3) + nrow(test4)

df.res.kruskal.adj <- rbind(test, test2, test3, test4)
df.res.kruskal.adj <- select(df.res.kruskal.adj, .y., n, df, p, p.adj, difference.p, different_result, method, vessel, Genus, Abundance )
write.csv2(df.res.kruskal.adj, "statistics_Kruskal_adjustment_BH.csv", row.names=FALSE, dec = ",", sep = ";")


# Genera
df.pwc.adj <- df.pwc 
df.pwc.adj2 <- data.frame()
for(i in c("P", "D")){
  for(x in c("ABS", "PROP")){
  test <- subset(df.pwc.adj, Vessel ==i); test <- subset(test, abundance ==x)
  testSTMT <- subset(test, group1 =="ST"); testSTMT  <- subset(testSTMT, group2 =="MT")
  testSTMT$p.adj.adj <- p.adjust(as.numeric(testSTMT$p.adj), method = "BH")
  
  
  testSTLT <- subset(test, group1 =="ST"); testSTLT <- subset(testSTLT, group2 =="LT")
  testSTLT$p.adj.adj <- p.adjust(as.numeric(testSTLT$p.adj), method = "BH")
  
  testMTLT <- subset(test, group1 =="MT"); testMTLT <- subset(testMTLT, group2 =="LT")
  testMTLT$p.adj.adj <- p.adjust(as.numeric(testMTLT$p.adj), method = "BH")
  
  df.pwc.adj2 <- rbind(df.pwc.adj2, testSTMT, testSTLT, testMTLT)
  }
}
df.pwc.adj <- df.pwc.adj2

df.pwc.adj$difference.p <- abs(df.pwc.adj$p.adj - df.pwc.adj$p.adj.adj)

test <- subset(df.pwc.adj, p.adj < 0.05 & p.adj.adj < 0.05);test$different_result <- "no_different_outcome"
test2 <- subset(df.pwc.adj, p.adj > 0.05 & p.adj.adj > 0.05); test2$different_result <- "no_different_outcome"
test3 <- subset(df.pwc.adj, p.adj < 0.05 & p.adj.adj > 0.05); test3$different_result <- "different_outcome"
test4 <- subset(df.pwc.adj, p.adj > 0.05 & p.adj.adj < 0.05); test4$different_result <- "different_outcome"
nrow(df.pwc.adj)
nrow(test) + nrow(test2) +nrow(test3) + nrow(test4)

df.pwc.adj <- rbind(test, test2, test3, test4)
df.pwc.adj <- select(df.pwc.adj, .y., group1, group2, n1, n2, statistic, p, p.adj, p.adj.adj, difference.p, different_result, p.adj.signif, Genus, Vessel, abundance, maxST, maxMT, maxLT)

write.csv2(df.pwc.adj, "statistics_Wilcoxon_Genera_BH.csv", row.names=FALSE, dec = ",", sep = ";")
# df.pwc.adj$p.adj.adj <- p.adjust(as.numeric(df.pwc.adj$p.adj), method = "BH")



# OTUs
df.pwc.adj <- df.pwc.otu
df.pwc.adj2 <- data.frame()
for(i in c("P", "D")){
  for(x in c("ABS", "PROP")){
  test <- subset(df.pwc.adj, Vessel ==i); test <- subset(test, abundance ==x)
...
df.pwc.adj <- rbind(test, test2, test3, test4)
df.pwc.adj <- select(df.pwc.adj, .y., group1, group2, n1, n2, statistic, p, p.adj, p.adj.adj, difference.p, different_result, p.adj.signif, Genus, Vessel, abundance)

write.csv2(df.pwc.adj, "statistics_Wilcoxon_OTUs_BH.csv", row.names=FALSE,  sep = ";", dec = ",")




# correlations
dfcor.adj <- data.frame()
for(i in c("P", "D")){
  for(x in c("Quantitative microbial community", "Proportional microbial community")){
  test <- subset(dfcor, PD_Colon ==i); test <- subset(test, comm ==x)
  
  test$p.adj <- p.adjust(as.numeric(test$pvalue), method = "BH")
  test$p <- test$pvalue
  
  dfcor.adj <- rbind(dfcor.adj, test)
  
  }
}

dfcor.adj$difference.p <- abs(dfcor.adj$p - dfcor.adj$p.adj)

test <- subset(dfcor.adj, p.adj < 0.05 & p < 0.05);test$different_result <- "no_different_outcome"
test2 <- subset(dfcor.adj, p.adj > 0.05 & p > 0.05); test2$different_result <- "no_different_outcome"
# test3 <- subset(dfcor.adj, p.adj < 0.05 & p > 0.05); test3$different_result <- "different_outcome"
test4 <- subset(dfcor.adj, p.adj > 0.05 & p < 0.05); test4$different_result <- "different_outcome"
nrow(dfcor.adj)
nrow(test) + nrow(test2) + nrow(test4)

dfcor.adj <- rbind(test, test2, test4)
dfcor.adj <- select(dfcor.adj, genus,cor,PD_Colon,p,p.adj,difference.p, different_result, sig, comm)
write.csv2(dfcor.adj, "statistics_Correlations_adjustment_BH.csv", row.names=FALSE, dec = ",", sep = ";")
