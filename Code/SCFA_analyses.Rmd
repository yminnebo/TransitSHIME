...
wdir <- function() setwd("INSERT HOME DIRECTORY HERE") #set working directory (=home)
saveplots <- function () setwd("INSERT DIRECTORY HERE") #set working directory to save plots
```
...
# Set whether to use black and white for figures
BW <- TRUE
...
wdir(); getwd()
functdata <- read.csv2("Dataframe_SCFA.csv", sep = ",")
...
functdata$Donor[functdata$Donor == 4] <- 7
functdata$Donor[functdata$Donor == 6] <- 4
...
functdata[, i] <- apply(functdata[, i], 2, function(x) as.numeric(as.character(x)))
...
# Add total_SCFA ratio of SCFA and branched to the dataframe
...
# Create dataframe without stabilization
...
functdatabruto <- functdatanet  # Copy the 'functdatanet' data to 'functdatabruto'

# Create data frames 'functdataP' and 'functdataD' by filtering 'functdatanet'
functdataP <- subset(functdatanet, Vessel == "P")
functdataD <- subset(functdatanet, Vessel == "D")

# Set a flag to control whether volume correction is applied or not
volcorr <- FALSE

# Check if 'volcorr' is TRUE
if (volcorr) {
  # Subsetting and adding a 'volume' column to 'functdataP' for different transit levels
  test1 <- subset(functdataP, Transit == "ST")
  test1$volume <- 0.200
  test2 <- subset(functdataP, Transit == "MT")
  test2$volume <- 0.400
  test3 <- subset(functdataP, Transit == "LT")
  test3$volume <- 0.600
  functdataP <- rbind(test1, test2, test3)
  
  # Subsetting and adding a 'volume' column to 'functdataD' for different transit levels
  test1 <- subset(functdataD, Transit == "ST")
  test1$volume <- 0.325
  test2 <- subset(functdataD, Transit == "MT")
  test2$volume <- 0.650
  test3 <- subset(functdataD, Transit == "LT")
  test3$volume <- 0.975
...
if (volcorr) {
  # Calculate proximal net production with volume correction
  functdataPnetnr <- subset(functdataP, select = c(Acetate:Lactate)) * (600 / 1000) * (1 / functdataP$volume)
} else {
  # Calculate proximal net production without volume correction
  functdataPnetnr <- subset(functdataP, select = c(Acetate:Lactate)) * (600 / 1000)
}

# Combine proximal net production with other selected columns
...
if (volcorr) {
  # Calculate distal net production with volume correction
  functdataDnetnr <- (subset(functdataD, select = c(Acetate:Lactate)) - subset(functdataP, select = c(Acetate:Lactate))) *
                    (600 / 1000) * (1 / functdataD$volume)
} else {
  # Calculate distal net production without volume correction
  functdataDnetnr <- (subset(functdataD, select = c(Acetate:Lactate)) - subset(functdataP, select = c(Acetate:Lactate))) *
                    (600 / 1000)
}

# Combine distal net production with other selected columns
...
## Calculate and Prepare Net Production Data

# Combine proximal and distal net production data along with 'FS' vessel data
functdatanet <- rbind(functdataPnet, functdataDnet, subset(functdatanet, Vessel == "FS"))

# Replace negative values with zero for specific columns
functdatanet[c(9:18)] <- replace(functdatanet[c(9:18)], functdatanet[c(9:18)] < 0, 0)

# Calculate total SCFA and ratios
...
functdatanet <- mutate(functdatanet, Acetateratio = Acetate/total_SCFA, Propionateratio = Propionate/total_SCFA, 
                       Butyrateratio = Butyrate/total_SCFA, Branched = Isovalerate + Isobutyrate, 
                       Branchedratio = Branched/total_SCFA, But_prop = Butyrate/Propionate)

## Analyze and Visualize Data
# Create data subsets for proximal and distal data
functdataP <- subset(functdatanet, Vessel == "P")
functdataD <- subset(functdatanet, Vessel == "D")

# Create dataframe without stabilisation and melt it
functdatatrt <- subset(functdatanet, Ninetransit == 3)
df.melted <- melt(functdatatrt, id.vars = c("Day", "Vessel", "Donor", "Transit"))

# Calculate means of metabolites per donor, Nutrient_Load, and PD_Colon
df.mean <- subset(functdatatrt, select = c("Day", "Vessel", "Donor", "Transit", "Butyrate", "Acetate", "Propionate", 
                                           "NH4.", "Lactate", "total_SCFA", "Branched", "Total_cells_events.ml"))
df.mean <- subset(df.mean, Vessel == "P" | Vessel == "D")

# Group and summarize mean data for specific variables
df.meanX <- group_by_at(df.mean, vars(Donor, Transit, Vessel))
df.meanX <- summarise_at(df.meanX, vars(Acetate, Propionate, Butyrate, NH4., Branched, total_SCFA,
                                        Lactate,Total_cells_events.ml), funs(sd, mean))

# Rename columns for consistency
colnames(df.meanX) <- c("Acet_sd", "Prop_sd", "Buty_sd", "NH4._sd", "Branched_sd", "SCFA_sd", "Lact_SD",
                        "Total_cells_events.ml_SD", "Acetate_mean", "Propionate_mean", "Butyrate_mean", 
                        "Ammonium_mean", "Branched_mean", "total_SCFA_mean", "Lactate_mean", 
                        "Total_cells_events.ml_mean")
...
# Subset data and add intermediate transit information
df.meanVFAnet <- subset(df.meanX, variable == "Acetate" | variable == "Propionate" | variable == "Butyrate")
df.meanVFAnet <- cbind(df.meanVFAnet, SD = df.meanSD$value, ybegin = df.meanbegin$value, yend = df.meanend$value)
df.meanVFAnet$intTransit <- ifelse(df.meanVFAnet$Donor %in% c(1, 2), "Short transit donor",
                                   ifelse(df.meanVFAnet$Donor %in% c(3, 4), "Medium transit donor", "Long transit donor"))
...
# Figure S15 - Correlations between BSS, Freq and corn
```{r Paperplot faecal samples, fig.height=8.5, fig.width=11}

## Check for Correlations and Visualize

# Create a donor data frame
df2 <- data.frame(Donor = c(1, 2, 3, 4, 5, 6))

# Add attributes for each donor
df <- subset(df2, Donor == 1)
df$Corntransit <- 21
df$cornmean <- 20
df$BSS <- 4
df$Frequency <- 7

# ... Repeat the above for other donors

# Combine donor data frames
...
# Assign 'intTransit' values based on donor
...
# Perform correlation test and format results
...
# Perform linear regression and summarize results
summary(lm(df$BSS ~ df$Corntransit))

# Create a scatter plot with custom theme
plot1 <- ggplot(df, aes(x = as.numeric(BSS), y = as.numeric(Corntransit), color = Donor,
                        shape = intTransit)) + 
         geom_point(size = 5, alpha = 0.8) + ylab("Corn transit time (h)") + 
         geom_smooth(color = "black", method = lm, inherit.aes = FALSE,
                     mapping = aes(x = as.numeric(BSS), y = as.numeric(Corntransit))) +
...
# Perform Spearman's correlation test between 'BSS' and 'Frequency' columns
rhot <- cor.test(df$BSS, df$Frequency, method = 'spearman')

# Format correlation result
rhot <- format(as.numeric(rhot$estimate[[1]]), digits = 3)

# Create text for correlation result
...
# Create a scatter plot with custom theme
plot2 <- ggplot(df, aes(x = as.numeric(BSS), y = as.numeric(Frequency), color = Donor,
                        shape = intTransit)) +
         geom_jitter(size = 5, alpha = 0.8) + ylab("Corn transit time (h)") +
         geom_smooth(color = "black", method = lm, inherit.aes = FALSE,
                     mapping = aes(x = as.numeric(BSS), y = as.numeric(Frequency))) +
         xlab("Bristol stool score") +
...
# Add color scaling for black and white mode
if (BW == TRUE) {
  plot2 <- plot2 + scale_color_grey(name = "Donor", start = 0, end = 0.6)
}
...
# Perform Spearman's correlation test between 'Corntransit' and 'Frequency' columns
...
# Create a scatter plot with custom theme
plot3 <- ggplot(df, aes(x = as.numeric(Corntransit), y = as.numeric(Frequency), color = Donor,
                        shape = intTransit)) +
         geom_point(size = 5, alpha = 0.8) + 
         geom_smooth(color = "black", method = lm, inherit.aes = FALSE,
                     mapping = aes(x = as.numeric(Corntransit), y = as.numeric(Frequency))) +
