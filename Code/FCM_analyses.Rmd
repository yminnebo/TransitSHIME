...
knitr::opts_chunk$set(
  ...
  include = TRUE,  # Include all content
  collapse = FALSE,
  dependson = NULL,
  warning = FALSE,
...
# Load packages for various functionalities
  ## Flow cytometry analysis and visualization
  library("Phenoflow")   # Provides fingerprinting tools
  library("flowViz")     # Offers advanced flow cytometry visualization
  library("ggcyto")      # Integrates ggplot2 and flow cytometry data
  library("plyr")        # Data manipulation functions
  library("dplyr")       # Data manipulation and transformation
  library("tibble")      # Data manipulation with tibbles

  ## Visualization and plotting
  library("ggplot2")     # Comprehensive plotting package
  library("cowplot")     # Arranging and annotating multiple plots
  library("ggpubr")      # Publication-ready plots with ggplot2
  library("gridExtra")   # Arranging multiple grid-based plots

  ## Data preprocessing and transformation
  library("reshape2")    # Data reshaping and melting

  ## Denoising flow cytometry data
  library("flowAI")      # Denoising flow cytometry data

  ## Data analysis and statistics
  library("rstatix")     # Pipe-friendly functions for statistical analysis
  library("tidyr")       # Data tidying and reshaping

  ## PowerPoint generation
  library("officer")     # Creating PowerPoint documents programmatically

  ## Custom source files for project-specific functions
  source("YM_KDP_papertheme.R")  # Custom theme for ggplot2
  source("gglocator2.R")         # Enhanced ggplot2 plot locator
  source("YM_extrafunctions.R")  # Additional custom functions
  source("YM_fusetoplayer.R")    # Project-specific function

# Paths and settings
paths <- list(
  "home" = "INSERT HOME DIRECTORY HERE", 
  "samples" = "INSERT DIRECTORY WITH FCS FILES HERE" # fcs. files are accessible on flow repository (see paper for more info) 
)

# Function to set plot saving directory (I like to have it exported in a different dir)
saveplots <- function() setwd("INSERT DIRECTORY HERE")

# Settings for generating PowerPoint and low-resolution plots
ppt = TRUE
low_resolution_plots = TRUE

# Unicode labels for in vivo
...
# create a biexponential transformation object function
...
## Load data
# The measured samples were analysed with a combined Sybr Green(SG) and Propidium iodide (PI) staining to differentiate the intact and damaged fractions.
# Split flowsets per SHIME run: 1 = donors 1 and 2, 2 = donors 3 and 6, 3 = donors 4 and 5:
...
df1 <- subset(df1, Samplename != "Tube_001_0_20190902_165147.fcs") # Remove due to insufficient cells in sample

df2 <- rbind(subset(FCM_FS_SGPI_metadata, Donor == "3"),subset(FCM_FS_SGPI_metadata, Donor == "4"))
df2 <- subset(df2, Samplename != "Tube_001_40_20191008_163744.fcs") # Remove due to insufficient cells in sample

df3 <- rbind(subset(FCM_FS_SGPI_metadata, Donor == "5"),subset(FCM_FS_SGPI_metadata, Donor == "6"))
df3 <- subset(df3, Samplename != "Tube_001_80.-5_20191106_160905.fcs") # Remove due to insufficient cells in sample
...
FCM_FS_SGPI_metadata$Donor[FCM_FS_SGPI_metadata$Donor == 7] <- 6
df2$Donor[df2$Donor == 4] <- 6; df3$Donor[df3$Donor == 6] <- 4
...
FCM_FS1@phenoData@data[["name"]] <- as.character(FCM_FS_SGPI_metadata[match(FCM_FS1@phenoData@data[["name"]],                                                                      
                                                                     as.character(FCM_FS_SGPI_metadata$Samplename)),]$Name)

sampleNames(FCM_FS2) <- as.character(FCM_FS_SGPI_metadata[match(flowFP::sampleNames(FCM_FS2),
                                       as.character(FCM_FS_SGPI_metadata$Samplename)),]$Name)
FCM_FS2@phenoData@data[["name"]] <- as.character(FCM_FS_SGPI_metadata[match(FCM_FS2@phenoData@data[["name"]],                                                                      
                                                                     as.character(FCM_FS_SGPI_metadata$Samplename)),]$Name)

sampleNames(FCM_FS3) <- as.character(FCM_FS_SGPI_metadata[match(flowFP::sampleNames(FCM_FS3),
                                       as.character(FCM_FS_SGPI_metadata$Samplename)),]$Name)
FCM_FS3@phenoData@data[["name"]] <- as.character(FCM_FS_SGPI_metadata[match(FCM_FS3@phenoData@data[["name"]],                                                                      
                                                                     as.character(FCM_FS_SGPI_metadata$Samplename)),]$Name)
...
## Gate samples - skip this chunk and run chunk Check gating split per SHIME run (line 419) if you want to reproduce data with the same gates as in the paper
...
# Load gates defined in the csv file. The gating used for the paper can be found in the datasets on the github repository!
 ## SHIME1
...
## Check gating
  plotthem = T # Name T if you want to plot all flow cytometry plots. F if you don't.
  if(plotthem == T){
...
The volumes of some samples could not be measured in SHIME run 2. TruCount beads were therefore used to measure them instead. This is already performed and put into a csv file (Sampledata_TruCount.csv) in the datasets
...
# Import fcs files
FCM_TruCount <- read.flowSet(transformation = FALSE, pattern=".fcs",path="INSERT PATH TO TruCount SAMPLES")
...
## Extract cell counts
# Extracts cell counts from the objects
...
## Create intermediate back up of data (just to be sure)
...
counts <- subset(counts, Filtered == "No")
counts <- subset(counts, Heat_killed == "No")
...
## Add a column with the in vivo transit time
...
# Biomass growth efficiency with carbohydrate consumptions; Growth efficiency per volumetric nutrient loading rate
## Start with averaged cell counts, grouped by date, donor, transit time and colon vessel 
...
print(papertheme(ggplot(netproduction, aes(x = Transit, y = Carbohydrate)) + 
  geom_bar(stat = "identity")+ 
  facet_grid(Vessel ~ Donor, scales = "free", 
    labeller = labeller(Vessel = Vessel.labs, Donor = donor.labs))))
```


## Perform the same but without averaged cell counts
```{r Biomass growth efficiency, fig.height=14, fig.width=14}
# Start with bruto production
netproduction <- subset(df.counts, Ninetransit == 3)
netproduction$comb <- paste0(netproduction$Date, netproduction$Donor, netproduction$Transit, netproduction$Vessel)
...
## Statistics
```{r}
correction <- "holm" # Favoured statistical correction method

# Calculate the cell count averages
...
# Non-parametric 1-way ANOVA without donor separation - Kruskal-Wallis
# Between transits
gglist <- list(); x = 1; df.pwc <- data.frame()

# Print a message indicating Kruskal-Wallis tests with correction
message(paste("Kruskal-Wallis tests with", correction, "correction between transit times"))

# Prepare the data for analysis
my_data <- biomass_relative_load; colnames(my_data)

# Ungroup the data to avoid problems
my_data <- ungroup(my_data)

# Perform Kruskal-Wallis test
rstatix::kruskal_test(my_data, Transit ~ Biomass)
res.kruskal <- my_data %>% rstatix::kruskal_test(Biomass ~ Transit)

# Calculate effect size and interpret the result with own function
effsize <- my_data %>% kruskal_effsize(Biomass ~ Transit)

if (effsize$effsize > 0.13) {
  effsize <- paste0(", effsize =", signif(effsize$effsize, digits = 5),
                    " (large effect size is detected)")
} else {
  if (effsize$effsize > 0.06) {
    effsize <- paste0(", effsize =", signif(effsize$effsize, digits = 5),
                      " (moderate effect size is detected)")
  } else {
    effsize <- paste0(", effsize =", signif(effsize$effsize, digits = 5),
                      " (WARNING: low effect size is detected)")
  }
}
...
# Print pairwise comparisons if needed
# print(pwc)

# Display Kruskal-Wallis test result and effect size
...
# Store pairwise comparison results in df.pwc.total
df.pwc.total <- df.pwc

# Display the stored pairwise comparison results
View(df.pwc.total)
...
df <- biomass_relative_load %>% group_by(Transit) %>% 
  summarise(SDtot = sd(TotalpermL), meantot = mean(TotalpermL), 
            SDPer = sd(Perc_intact), meanperc = mean(Perc_intact),
            Transit = Transit, Vessel = Vessel)
df <- df[!duplicated(df), ]

```

## Make plots of microbial load
...
saveplots() # Change directory to save plots
...
## Statistics - carbohydrate utilisation
```{r}
correction <- "holm"
...
## Figure 1D and 1E - biomass-carbohydrates
```{r Biomass growth efficiency, fig.height=7, fig.width=16}
...
paperplot_fig1D <- plotA
paperplot_fig1E <- plotB

ggarrange(paperplot_fig1D, paperplot_fig1E)
```
...
# Figure S7 - Paperplot net daily carbohydrate utilisation 
...
# Filter the data and set Donor as an ordered factor
my_data <- subset(df.counts, Vessel == "P")
my_data <- subset(my_data, Ninetransit == 3)
...
# Generate frequency tables for Donor and Transit
...
# Add mean and dotplot to the boxplot
...
# Perform a two-way ANOVA with interaction effect
res.aov3 <- aov(TotalpermL ~ Transit * Donor, data = my_data)
summary(res.aov3)

# Perform post-hoc Tukey multiple pairwise-comparisons for Donor and Transit
...
# 2. Normality - Shapiro-Wilk test for residuals
plot(res.aov3, 2)
aov_residuals <- residuals(object = res.aov3)
t <- shapiro.test(x = aov_residuals)
if (t[["p.value"]] < 0.05) {
  message(crayon::red(paste0("p=", t[["p.value"]], ", Data is not normally distributed!")))
} else {
...
# Set the correction method for p-values
correction <- "holm"
...
# Perform Kruskal-Wallis tests for different vessels and donors
for (i in c("P", "D")) {
  my_data <- subset(df.counts, Vessel == i)
  my_data <- subset(my_data, Ninetransit == 3)
  
  # Update the vessel label for readability
  if (i == "P") { i_label <- "Proximal colon" }
  if (i == "D") { i_label <- "Distal colon" }
  ...
  for (j in c(1, 2, 3, 4, 5, 6)) {
    ...
    if (effsize$effsize > 0.13) {
      effsize_label <- paste0(", effect size = ", signif(effsize$effsize, digits = 5), " (large effect)")
    } else {
      if (effsize$effsize > 0.06) {
        effsize_label <- paste0(", effect size = ", signif(effsize$effsize, digits = 5), " (moderate effect)")
      } else {
        effsize_label <- paste0(", effect size = ", signif(effsize$effsize, digits = 5), " (low effect)")
      }
    }
    
    # Perform pairwise comparisons using Wilcoxon’s test
    ...
    if (res.kruskal$p < 0.05) {
      kruskal_result <- crayon::green(paste0("Kruskal-Wallis: Donor ", j, ", ", i_label, 
                                              ", p =", res.kruskal$p, effsize_label))
    } else {
      kruskal_result <- crayon::red(paste0("Kruskal-Wallis: Donor ", j, ", ", i_label, 
                                            ", p =", res.kruskal$p, effsize_label))
    }
    message(kruskal_result)
    
    # Visualization: box plots with p-values
    pwc <- pwc %>% add_xy_position(x = "Transit")
    gglist[[x]] <- ggboxplot(my_data2, x = "Transit", y = "TotalpermL") +
      stat_pvalue_manual(pwc, hide.ns = TRUE) +
      labs(title = paste0("Donor ", j, ", ", i_label),
           subtitle = get_test_label(res.kruskal, detailed = TRUE),
           caption = get_pwc_label(pwc))
    x <- x + 1
  }
...
# Perform Kruskal-Wallis tests between donors
for (i in c("P", "D")) {
  my_data <- subset(df.counts, Vessel == i)
  my_data <- subset(my_data, Ninetransit == 3)
  ...
  for (j in c("ST", "MT", "LT")) {
    # Subset data for the specific transit
    ...
    pwc$Vessel <- i_label
    pwc$Transit <- j
    df.pwc1 <- rbind(df.pwc1, pwc)
    
    # Print Kruskal-Wallis test result
    if (res.kruskal$p < 0.05) {
      kruskal_result <- crayon::green(paste0("Kruskal-Wallis: Transit ", j, ", ", i_label, 
                                              ", p =", res.kruskal$p, effsize_label))
    } else {
      kruskal_result <- crayon::red(paste0("Kruskal-Wallis: Transit ", j, ", ", i_label, 
                                            ", p =", res.kruskal$p, effsize_label))
    }
    message(kruskal_result)
    ...
    gglist[[x]] <- ggboxplot(my_data2, x = "Donor", y = "TotalpermL") +
      stat_pvalue_manual(pwc, hide.ns = TRUE) +
      labs(title = paste0("Transit ", j, ", Vessel ", i_label),
           subtitle = get_test_label(res.kruskal, detailed = TRUE),
           caption = get_pwc_label(pwc))
    x <- x + 1
  }
...
for (i in c("P", "D")) {
  ...
  if (i == "P") {
    i_label <- "Proximal colon"
  } else if (i == "D") {
    i_label <- "Distal colon"
  }
  ...
  pwc <- my_data2 %>% wilcox_test(TotalpermL ~ Transit, p.adjust.method = correction, paired = FALSE)
  pwc <- subset(pwc, p.adj.signif != "ns")
  pwc$Vessel <- i_label
  pwc$Transit <- j
  df.pwc <- rbind(df.pwc, pwc)
  ...
  if (res.kruskal$p < 0.05) {
    kruskal_result <- crayon::green(paste0("Kruskal-Wallis: ", i_label, ", p =", res.kruskal$p, effsize_label))
  } else {
    kruskal_result <- crayon::red(paste0("Kruskal-Wallis: ", i_label, ", p =", res.kruskal$p, effsize_label))
  }
  message(kruskal_result)
  ...
  gglist[[x]] <- ggboxplot(my_data2, x = "Transit", y = "TotalpermL") +
    stat_pvalue_manual(pwc, hide.ns = TRUE) +
    labs(title = paste0("Donor ", j, ", Vessel ", i_label),
         subtitle = get_test_label(res.kruskal, detailed = TRUE),
         caption = get_pwc_label(pwc))
  x <- x + 1
}
...
for (j in c("ST", "MT", "LT")) {
  my_data2 <- subset(df.counts, Transit == j)
  my_data2 <- subset(my_data2, Ninetransit == 3)
  
  res.kruskal <- my_data2 %>% rstatix::kruskal_test(TotalpermL ~ Vessel)
  test1 <- subset(my_data2, Vessel == "P")
  test <- subset(my_data2, Vessel == "D")
  ...
  pwc <- my_data2 %>% wilcox_test(TotalpermL ~ Vessel, p.adjust.method = "bonferroni", paired = FALSE)
  pwc <- subset(pwc, p != "ns")
  pwc$Vessel <- i
  pwc$Transit <- j
  df.pwc1 <- rbind(df.pwc1, pwc)
  ...
  if (res.kruskal$p < 0.05) {
    kruskal_result <- crayon::green(paste0("Kruskal-Wallis: Transit ", j, ", p =", res.kruskal$p, effsize_label))
  } else {
    kruskal_result <- crayon::red(paste0("Kruskal-Wallis: Transit ", j, ", p =", res.kruskal$p, effsize_label))
  }
  message(kruskal_result)
...
for (i in c("P", "D")) {
  ...
  pwc <- my_data2 %>% wilcox_test(Perc_intact ~ Transit, p.adjust.method = correction, paired = FALSE)
  pwc <- subset(pwc, p.adj.signif != "ns")
  pwc$Vessel <- i_label
  pwc$Transit <- j
  df.pwc <- rbind(df.pwc, pwc)
...
for (j in c("ST", "MT", "LT")) {
  ...
  pwc <- my_data2 %>% wilcox_test(Perc_intact ~ Vessel, p.adjust.method = "bonferroni", paired = FALSE)
  pwc <- subset(pwc, p != "ns")
  pwc$Vessel <- i
  pwc$Transit <- j
  df.pwc1 <- rbind(df.pwc1, pwc)
...
df <- subset(df.counts, Ninetransit == 3)
df$comb <- paste0(df$Transit, df$Vessel)
...
df <- df %>%
  ...
  summarise(
    SDtot = sd(TotalpermL),
    meantot = mean(TotalpermL),
    SDPer = sd(Perc_intact),
    meanperc = mean(Perc_intact),
    Transit = Transit,
    Vessel = Vessel
  )
df <- df[!duplicated(df), ]
...
for (i in 1:nrow(df)) {
  ...
  message(paste0("Transit: ", df2$Transit, ", Vessel: ", df2$Vessel, " ",
                 format(df2$meantot, scientific = TRUE), " +- ", format(df2$SDtot, scientific = TRUE)))
}
...
plotD
paperplot_fig1A <- plotD
```

# Figure 1 - Paperplot combination faecal, correlation and dotplot
```{r fig.height=14, fig.width=17}

paperplot_fig1B <- paperplot_dotplot + theme(axis.text.y = element_text(size = 20,family='sans',angle=0),
                         axis.title.y = element_text(size=20,family='sans',angle=90),
                         axis.ticks.y =element_line(colour = "black"))


comboplot <- ggarrange(
          ggarrange(paperplot_fig1A + guides(alpha = "none", fill=guide_legend(ncol=1, title.position = "top"), 
            color=guide_legend(ncol = 2, title.position = "top", title.hjust = 0.5, override.aes = list(size=5))) +
              theme(legend.position = c(0.65, 0.13), legend.box = "horizontal", legend.title = element_text(size = 19),
                    legend.text = element_text(size = 19)), 
                    paperplot_fig1B + ylab(expression(paste("Cell concentration (cells ", mL^{"-1"}, ")"))) + 
              scale_x_discrete(labels= c("Short", "Med", "Long")),
                    paperplot_fig1C,  
                    labels = c("A", "B", "C"), font.label = list(size =22), ncol = 3, widths = c(1.45,0.87,1.5)),
          ggarrange(paperplot_fig1D,
                    ggarrange(NULL,paperplot_fig1E + scale_x_discrete(labels= c("Short", "Med", "Long")), 
                              nrow = 2, heights = c(0.05,1)), 
                    paperplot_fig1F +scale_x_discrete(labels= c("Short", "Med", "Long")),
